...
argparser.add_argument("-d", "--dryrun", type=int, default=0)
...
for album in albums:
    ...
    if len(meta) == 2:
        ...
        search_soup = BeautifulSoup(requests.get("https://bandcamp.com/search?q="+title).content, "html.parser")
        ...
        for result in results:
            ...
            artist_similar = SequenceMatcher(None, entry_artist, artist).ratio() >= .5
            album_similar = SequenceMatcher(None, entry_name, title).ratio() >= .8
            ...
            is_match = artist_similar and album_similar
